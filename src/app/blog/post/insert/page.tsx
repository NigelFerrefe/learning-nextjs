...
export default function Page() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const uuid = uuidv4();
    fetch(
      `/api/posts?id=${uuid}&title=${formData.title}&author=${user?.name}&content=${content || formData.content}&date=${formData.date}`,
      {
        method: "POST",
        headers: {
    ...
    <div className="bg-white p-8 rounded shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <textarea id="content" name="content"  value={postContent} onChange={handleChange} className="w-full border-2 border-purple-100 p-2 rounded-md focus:border-purple-200 focus:outline-none"></textarea>
